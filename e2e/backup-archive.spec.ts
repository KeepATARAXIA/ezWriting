...
async function openData(page: Page) {
  const data = page.getByRole('button', { name: '设置', exact: true })
...
test('downloads deduplicated media, restores playable video in a separate origin, and rejects damaged media', async ({ page }, info) => {
  ...
  await expect(page.locator('.topbar-document-title')).toHaveText('媒体备份验收')
...
test('cancel remains clickable above the local user menu and does not download a partial archive', async ({ page }, info) => {
  await page.goto('/')
  ...
  await page.reload()
  await expect(page.locator('.topbar-document-title')).toHaveText('取消备份验收')
})
