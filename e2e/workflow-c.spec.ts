...
test('classifies resources, shows export caveats, and copies X original images', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.notification-panel')).toContainText('GIF 将输出为静态图片')
  ...
  await page.getByRole('navigation', { name: '手机工作区' }).getByRole('button', { name: '编辑', exact: true }).click()
  await page.getByRole('button', { name: '文档素材', exact: true }).click()
  await expect(page.locator('.article-resource-card')).toHaveCount(3)
