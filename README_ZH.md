...
- 本地图片的插入、替换、删除和说明编辑。
- 本地 MP4 / WebM 视频插入与播放，单文件上限 100 MiB；视频只在左侧编辑器中播放，右侧平台预览使用稳定的静态画面，需要平台原生上传时会给出人工处理提示。
...
- 大视频会在整库备份中重复内嵌为 Base64，可能超过当前 128 MiB 的备份恢复上限。请保留原始视频，不要仅凭已导出超限备份就清除浏览器数据。
- 默认保存为草稿，不提供无人值守发布。
